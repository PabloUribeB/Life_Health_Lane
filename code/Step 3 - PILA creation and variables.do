...
foreach ocupacion in $ocupaciones {
    ...
    * Only use individuals without a second major
    use if mi(rethus_codigoperfilpre2) & rethus_codigoperfilpre1 ==         ///
        "`ocupacion'" using "${data_rethus}\RETHUS_procesada.dta", clear
    ...
    compress
    tempfile temp_`ocupacion'
    save `temp_`ocupacion''


    * Create empty dataset where each month will be appended recursively
    clear all
    gen delete = .
    save "${data}\\`ocupacion'_PILA_monthly.dta", replace
    ...
    * Loop through PILA
    forval y=2008/2022 {
        ...
        foreach i of global loop {
            ...
            use personabasicaid id ibc* salario_bas sal_dias_cot tipo_cotiz ///
                sexomode fechantomode ciudad_cod depto_cod licen_mat        ///
                incap_gral incap_trab using "`y'm`i'cBR", clear
                
            compress
            
            merge m:1 personabasicaid using `temp_`ocupacion'', keep(3) nogen
            ...
            *New variables
            ...
            gen     pila_independientes = inlist(tipo_cotizante, 2, 3, 16,  ///
                                          41, 42, 59, 57, 66)
            ...
            *Maternity license
            gen licen_mat1 = (licen_mat == "X")
            drop licen_mat
            rename licen_mat1 licen_mat
            
            *General incapacity
            gen incap_gral1 = (incap_gral == "X")
            drop incap_gral
            ...
            *Minimum wage t-1
            ...
            label var pila_salario "Salario nominal"
            drop 	rowmax
            
            gen		pila_salario_max = pila_salario
            
            *Get the CPI
            merge m:1 year month using "${ipc}\IPC mensual", keepusing(IPC) ///
                  keep(1 3) nogen
            ...
            * Since people may have more than one contribution each month, 
            * sum the wages of each contribution and keep the max of worked days. 
            ...
            foreach var of varlist sal_dias_cot pila_dependientes           ///
                pila_independientes pila_posgrad_salud pila_salario_max_r   ///
                incap_dias incap_gral licen_mat {
            ...
            replace ciudad_cod 	= "00" + ciudad_cod if      ///
                    (length(ciudad_cod) == 1 & ciudad_cod != ".")
                    
            replace ciudad_cod 	= "0" + ciudad_cod 	if      ///
                    (length(ciudad_cod) == 2 & ciudad_cod != ".")
            
            replace depto_cod 	= "0" + depto_cod 	if      ///
                    (length(depto_cod) == 1 & depto_cod != ".")
            
            gen pila_cod_mun 	= depto_cod + ciudad_cod
            
            *Keep relevant variables
            keep personabasicaid fecha_pila year month sexomode             ///
            fechantomode *_r pila_independientes pila_dependientes          ///
            pila_cod_mun id sal_dias_cot *dependientes *independientes      ///
            nro_cotizaciones pila_posgrad_salud rethus_sexo                 ///
            rethus_posgrado year_posgrado fechalicenciapregrado             ///
            year_licencia fechapregrado year_grado rethus_fechagradopos1    ///
            year_posgrado rethus_codigoperfilpos1 rethus_perfilpos1         ///
            tipo_cotiz rethus_codigoperfilpre1 pila_salario_max_r           ///
            incap_dias incap_gral licen_mat
            ...
            * Append observations to the file created at the top
            append using "${data}\\`ocupacion'_PILA_monthly.dta"
            cap drop delete
            
            * Save that file so in the next iteration, the new observations 
