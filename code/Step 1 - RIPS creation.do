...
drop if !mi(rethus_codigoperfilpre2) // Drop individuals with more than one major

* Keep relevant majors only
* P01 Bacteriologists, P03 Nurses, P07 Physicians, P09 Dentists
keep if inlist(rethus_codigoperfilpre1,"P01","P03","P07","P09")
...
forvalues year = 2009/2021 {
    ...
    foreach base of global b {
        ...
        if "`base'" == "consultas" | "`base'" == "procedimientos" {
            ...
            if `year' < 2019 {
                ...
                forvalues i = 1/`max_i' {
                    ...
                    use personabasicaid `diag' `date' `extra_diag' `causa'  ///
                    `consul' using "${RIPS2}\\`base'`vari_id'`year'_`i'", clear
        ...
        * Merge the module that is currently in memory to our main sample
        merge m:1 personabasicaid using "${data}\master_rethus.dta",    ///
              keep(3) nogen
        ...
        * Save that file so in the next iteration, the new observations are 
        * part of the file
        save "${data}\RIPS\sample_`year'_RIPS", replace
    }
}
...
* The following loop does the same as above but using TXT files instead of 
* dta to call the raw data.
...
forvalues i = 1/8 {
    ...
    * Consultations
    if `i' == 3 | `i' == 4 {
        ...
        rename (cod_diag_r1 cod_diag_r2 cod_diag_r3)                ///
               (diag_r1 diag_r2 diag_r3)
