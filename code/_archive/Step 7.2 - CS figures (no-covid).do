...
use "${output}\CS_Results_nocovid.dta", clear
...
gen pila_outcome 	=  (outcome == "sal_dias_cot_0"			| outcome == "pila_salario_r_0" 		|				///
						outcome == "posgrado_salud"			)
						
gen rips_outcome	=  (outcome == "urg"					| outcome == "hosp"				    	| 				///
						outcome == "service_mental_forever"	)
...
* Relevant periods	
drop if (dist < -1 & pila_outcome == 0)
drop if (dist < -4 & pila_outcome == 1)

drop if (dist >  2 & pila_outcome == 0)
drop if (dist >  6 & pila_outcome == 1)
...
foreach var in max min {
	...
	rename round_number round_number_`var'
	rename decimal decimal_`var'
	rename unit unit_`var'
...
foreach outcome in `outcomes' {
	...
	foreach gender in `genders' {
		...
		twoway 	(rspike ci_lower ci_upper dist 	if (occupation == "P01" & outcome == "`outcome'"), lcolor(gs11) color(gs11) lp(solid))					///
				...
				xlabel(-1(1)2, nogrid labsize(small))	 																							 	///
		...
		graph export "${figures}\Callaway SantAnna (no-covid)\ES_`outcome'_`gender'.png", replace
...
foreach outcome in `outcomes' {
	...
	foreach gender in `genders'  {
		...
		twoway 	(rspike ci_lower ci_upper dist 	if (occupation == "P01" & outcome == "`outcome'"), lcolor(gs11) color(gs11) lp(solid))					///
				...
				xlabel(-4(1)6, nogrid labsize(vsmall))																									///
...
foreach outcome in `outcomes' {
		...
		graph export "${figures}\Callaway SantAnna (no-covid)\ES_`outcome'_gap.png", replace
